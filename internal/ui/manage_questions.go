...
func (ui *UI) ManageQuestions() {
	for {
		...
		fmt.Println(utils.Colorize("          MANAGE QUESTIONS          ", "cyan", "bold"))
		fmt.Println(utils.Colorize("====================================", "cyan", "bold"))
		fmt.Println(utils.Colorize("1. Add questions", "", ""))
		fmt.Println(utils.Colorize("2. Remove question", "", ""))
		fmt.Println(utils.Colorize("3. Go back", "", ""))
...
func (ui *UI) AddQuestions() {
	...
	if err != nil {
		fmt.Println(utils.Colorize("Error reading directory", "red", "bold"))
		return
	}

	if len(files) == 0 {
		fmt.Println(utils.Colorize("No files found in the directory.", "yellow", "bold"))
		return
	}

	fmt.Println(utils.Colorize("Select a file to add questions from:", "cyan", "bold"))
	...
	for {
		...
		fmt.Print(utils.Colorize("Enter the file number: ", "yellow", "bold"))
		...
		if filepath.Ext(fileName) != ".csv" {
			fmt.Println(utils.Colorize("Selected file is not a CSV file.", "red", "bold"))
			continue
		}
		break
	}
	...
	if err != nil {
		fmt.Println(utils.Colorize("Error adding questions from file:", "red", "bold"), fileName, err)
		return
	} else if !newQuestionsAdded {
		fmt.Println(utils.Colorize("No new questions in the file:", "yellow", "bold"), fileName)
	} else {
		fmt.Println(utils.Colorize("Questions successfully added from file:", "green", "bold"), fileName)
	}
...
func (ui *UI) RemoveQuestion() {
	...
	var err error
	for {
		fmt.Print(utils.Colorize("Enter the Question ID to remove: ", "yellow", "bold"))
	...
	if err != nil {
		fmt.Println(utils.Colorize("Failed to remove the question:", "red", "bold"), err)
		return
	} else {
		fmt.Println(utils.Colorize("Question removed successfully!", "green", "bold"))
	}
